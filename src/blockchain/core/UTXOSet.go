		...
		for index, output := range tx.Vouts {
			...
			for _, input := range inputs {
				if bytes.Compare(tx.TxID, input.TxID) == 0 && index == input.Vout {
					if bytes.Compare(output.PubKeyHash, wallet.PubKeyHash(input.PublicKey)) == 0 {
						isSpent = true
					}
				}
			}
	...
	err := utxoSet.BlockChian.DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			...
			for _, input := range inputs {
				...
				for _, utxo := range txOutputs.UTXOs {
					if bytes.Compare(utxo.Output.PubKeyHash, wallet.PubKeyHash(input.PublicKey)) == 0 && input.Vout == utxo.Index {
						isNeedDelete = true
					} else {
						utxos = append(utxos, utxo)
					}
