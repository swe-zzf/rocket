...
func CreateBlockChainWithGenesisBlock(address string) {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		//创世区块序列化后，存入到数据库中
		b, err := tx.CreateBucketIfNotExists([]byte(util.BlockBucketName))
		if err != nil {
			log.Panic(err)
		}
...
//提供一个方法，用于判断数据库是否存在
func dbExists() bool {
	if _, err := os.Stat(util.DBName); os.IsNotExist(err) {
		return false
	}
	return true
}
...
func GetBlockChainObject() *BlockChain {
	...
	if dbExists() {
		...
		err = db.View(func(tx *bolt.Tx) error {
			...
			b := tx.Bucket([]byte(util.BlockBucketName))
			if b != nil {
				//读取最新hash
				hash := b.Get([]byte("l"))
...
func caculate(tx *Transaction, address string, spentTxOutputMap map[string][]int, unSpentUTXOs []*UTXO) []*UTXO {
	...
	if !tx.IsCoinBaseTransaction() { //tx不是CoinBase交易，遍历TxInput
		for _, txInput := range tx.Vins {
			//txInput-->TxInput
			full_payload := util.Base58Decode([]byte(address))

			pubKeyHash := full_payload[1 : len(full_payload)-wallet.AddressCheckSumLen]
