...
import (
	"flag"
	"github.com/swe-zzf/rocket/src/blockchain/util"
...
func (cli *CLI) Run() {
	...
	*/
	...
	if CreateBlockChainCmd.Parsed() {
		if !wallet.IsValidAddress([]byte(*flagCreateBlockChainData)) {
	...
	if sendCmd.Parsed() {
		...
		//添加区块
		from := util.JSONToArray(*flagSendFromData)     //[]string
		to := util.JSONToArray(*flagSendToData)         //[]string
		amount := util.JSONToArray(*flagSendAmountData) //[]string
		for i := 0; i < len(from); i++ {
			if !wallet.IsValidAddress([]byte(from[i])) || !wallet.IsValidAddress([]byte(to[i])) {
	...
	if getBalanceCmd.Parsed() {
		if !wallet.IsValidAddress([]byte(*flagGetBalanceData)) {
			fmt.Println("查询地址有误")
			printUsage()
			os.Exit(1)
		}
