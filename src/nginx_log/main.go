...
func init() {
	flag.StringVar(&path, "path", "/app/tmp/nginx-access.log", "log file path")
	// influxDsn: http://ip:port@username@password@db@时间精度毫秒ms
	flag.StringVar(&influxDsn, "influxDsn", "http://127.0.0.1:31086@username@password@test@ms", "influxDB dsn")
	flag.StringVar(&listenPort, "listenPort", "8888", "monitor port")
